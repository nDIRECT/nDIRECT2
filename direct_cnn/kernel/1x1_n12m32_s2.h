...
void avx512_1x1s2(int H, int W, int N, int C, float *input, int K, int R, float *filter, float *output)
{
    int S = R;
    ...
    int Tm = NUM / Tn;          // 1
    //printf("entry begin   \n");
    //posix_memalign(&ptr, 64, NUM * CONV_C_1X1 * 32 * sizeof( float ));

    float *NDIRECT_input = (float*)_mm_malloc((size_t)NUM * CONV_C_1X1 * 32 *sizeof(float), 64);
    
    //printf("filter malloc error!\n");
    //printf("entry end   \n");
    //posix_memalign(&ptr1, 64, CONV_C_1X1 * Tm * CONV_K * sizeof( float ));
    ...
    // -----------note------------------
    //int Num_K_block = (K / Tm + CONV_K_1X1 - 1) / CONV_K_1X1;
    //int Num_C_block = (C + CONV_C_1X1 - 1) / CONV_C_1X1;
    ...
    #pragma omp parallel num_threads(NUM)
    {
        ...
        for ( jj = jjs; jj < H_to; jj = jj + Hb)          // 0 gride_HW_size HWb
        {
            ...
            //vk * CONV_C_1X1 * (K/vk) * (C/CONV_C_1x1)
            for (cc = ccs; cc < C_to; cc = cc + Cb)         // 0 C Cb
            {

                //C_index = cc / CONV_C_1X1;
                Cb = CONV_C_1X1;
                ...
                for ( ii = iis ; ii < K_to; ii = ii + Kb)   // 0 K Kb
                {
                    ...
                    //pre_index = C_index * Num_K_block * Tn + (ii - iis) / CONV_K_1X1 * Tn;
                    Kb = CONV_K_1X1;
                        ...
                        for(hs = 0; hs < Hb; hs++){
                            ...
                            if(EDGE_Wb != 0)      // 4 1
                            {
                                ...
                                if (EDGE_Wb == 56)
                                {
                                    ...
                                    direct_1x1_N12M28_s2(temp_buffer_output, buffer_filter1, temp_buffer_input, D_Kb, Cb, input_HW_size<<2, &NDIRECT_input[id * CONV_C_1X1 * 32], cc, EDGE_Kb);                            }

                                else if (EDGE_Wb == 28)
                                {
                                    //printf("28 hs: %d, cc: %d, ii: %d, Kb: %d, EDGE_Kb: %d \n", hs,cc, ii, Kb, EDGE_Kb);
                                    direct_1x1_N12M14_s2(temp_buffer_output, buffer_filter1, temp_buffer_input, D_Kb, Cb, input_HW_size<<2, &NDIRECT_input[id * CONV_C_1X1 * 32], cc, EDGE_Kb);                            }
                                else if (EDGE_Wb == 14)
                                {
                                    //printf("14 hs: %d, cc: %d, ii: %d, Kb: %d, EDGE_Kb: %d \n", hs,cc, ii, Kb, EDGE_Kb);
                                    direct_1x1_N12M7_s2(temp_buffer_output, buffer_filter1, temp_buffer_input, D_Kb, Cb, input_HW_size<<2, &NDIRECT_input[id * CONV_C_1X1 * 32], cc, EDGE_Kb);                            }
                            }
