...
void avx512_Kernel_conv_nchw_7x7s2(float *filter, float *input, float *output, float *output_buffer, int K, int Kb, int H, int R, int Cb)
{
    ...
    #pragma omp parallel num_threads(NUM)
    {
        ...
        for(cc = 0; cc < C; cc = cc + Cb)
        {
            ...
            for(ii = 0; ii < K; ii = ii + Kb)
            {
                ...
                avx512_Kernel_conv_nchw_7x7s2(filter0, input0, output0, output_buffer0, K, Kb, H, R, Cb);
            }
        }
